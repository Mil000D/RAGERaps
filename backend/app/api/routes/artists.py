...
async def process_csv_content(request: CSVContentRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to process CSV content: {str(e)}"
        )
...
async def search_artists_get(
    ...
    filter_by_genre: Optional[str] = Query(
        None, description="Filter by genre (comma-separated for multiple)"
    ),
...
async def get_artists_by_genre(
    genre: str, k: int = Query(10, description="Number of results to return")
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get artists by genre: {str(e)}"
        )
